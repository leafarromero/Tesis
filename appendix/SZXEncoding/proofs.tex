...
\begin{lemma}%
    ...
    Given type judgements $\Phi, x:A \vdash M : B$, and $\Phi\vdash N : A$. Then, $\natEval{M}_{x:A,\Phi} (\natEval{N}_{\Phi},|\Phi|)= \natEval{M[N/x]}_\Phi($ $|\Phi|$ $)$.
\end{lemma}
...
\begin{lemma}{\!\textbf{(\ref{lem:eval-type})}}
    ...
    $\natEval{M}_\Phi \in \bigtimes\limits_{x:P \in \Phi} \natEval{P} \to \natEval{A}$.
\end{lemma}

\begin{proof}
    By induction on the typing judgement $\Phi \vdash M : A$:
    \begin{itemize}
        \item If $\Phi\vdash n:\nat$, then $\natEval{n}_\Phi = |\Phi| \mapsto n \in  \bigtimes\limits_{x:P \in \Phi} \natEval{P} \to \N$.

        \item If $\Phi, x:A\vdash x:A$, then $\natEval{x}_{x:A, \Phi} = x,|\Phi| \mapsto x \in \bigtimes\limits_{y:P \in x:A,\Phi} \natEval{P} \to \natEval{A}$.
        ...
        Then $\natEval{\ifz{L}{M}{N}}_\Phi\in\bigtimes\limits_{x:P \in \Phi} \natEval{P} \to \natEval{A}$.
        ...
        $x\mapsto\natEval{M}_\Phi(x, \Phi)\in \natEval{k:\nat\to A[k]}$. Then $|\Phi|
        \mapsto \bigtimes\limits_{k\in \natEval{N}_\Phi(|\Phi|)} \natEval{M}_{k:\nat\Phi}(k,
        |\Phi|)\in\bigtimes\limits_{x:P \in \Phi} \natEval{P} \to\natEval{A[k/N]}^n$.

        \item If $\Phi\vdash \Qlet{x^B : y^{\typeVec B n}}{M}{N}:A$, then
        $\natEval{\Qlet{x^P :: y^{\typeVec{P}{n}}}{M}{N}}_\Phi =$\allowbreak
        $|\Phi|\mapsto \natEval{N}_{x:P, y:\typeVec{P}{n}\Phi}(y_1,[y_2,\dots,y_n],|\Phi|)$ where $[y_1,\dots,y_n] = \natEval{M}_\Phi(|\Phi|)$.
        
        By inductive hypothesis
        $\natEval{M}_\Phi(|\Phi|)\in \natEval{B}^n$ and $\natEval{N}_{x:P, y:\typeVec{P}{n}
        \Phi}(y_1,[y_2,\dots,y_n],|\Phi|)\in\natEval{A}$. Then $|\Phi|\mapsto
        \natEval{N}_{x:P, y:\typeVec{P}{n}
        \Phi}(y_1,[y_2,\dots,y_n],|\Phi|)\in\bigtimes\limits_{x:P \in \Phi} \natEval{P}
        \to \natEval{A}$.

        \item If $\Phi\vdash \Qrange : (n:\nat)\to (m:\nat)\to \typeVec{\nat}{(m-n)}$, then $\natEval{\Qrange}_\Phi = n,m,|\Phi| \mapsto \bigtimes\limits_{i=n}^{m-1} i\in \bigtimes\limits_{z:P \in x:\nat,y:\nat,\Phi} \natEval{P} \to \N^{m-n}$
    \end{itemize}
\end{proof}
...
\begin{proof}
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            ...
            \begin{align*}
                \natEval{\Qfor{k}{M'}{R}}_\Phi &= |\Phi| \mapsto \bigtimes\limits_{k\in \natEval{M'}_\Phi(|\Phi|)} \natEval{R}_\Phi(k, |\Phi|)\\
                &= |\Phi| \mapsto \bigtimes\limits_{k\in \natEval{N}_\Phi(|\Phi|)} \natEval{R}_\Phi(k, |\Phi|)\\
                &= \natEval{\Qfor{k}{N}{R}}_\Phi
            \end{align*}

            \item If $M\to R[k/M_1]\ :: \Qfor{k}{M_2}{R}$ with $M' = V::L$, then: 
            \begin{align*}
                &\hspace*{-25pt}\natEval{\Qfor{k}{M_1\ ::\ M_2}{R}}_\Phi \\
                &= |\Phi| \mapsto \bigtimes\limits_{k\in \natEval{M_1\ ::\ M_2}_\Phi(|\Phi|)} \natEval{R}_\Phi(k, |\Phi|)\\
                &= |\Phi| \mapsto \bigtimes\limits_{k\in \natEval{M_1}_\Phi(|\Phi|) \times \natEval{M_2}_\Phi(|\Phi|)} \natEval{R}_\Phi(k, |\Phi|)\\
                &= |\Phi| \mapsto \natEval{R}_{k,\Phi}(\natEval{M_1}_\Phi(|\Phi|), |\Phi|) \times \bigtimes\limits_{k\in \natEval{M_2}_\Phi(|\Phi|} \natEval{R}_{k,\Phi}(k, |\Phi|)\\
            ...
            \begin{align*}
                \natEval{\Qfor{k}{\vnil^\nat}{R}}_\Phi &= |\Phi| \mapsto \bigtimes\limits_{k\in \natEval{\vnil^\nat}_\Phi(|\Phi|)} \natEval{R}_\Phi(k, |\Phi|)\\
                &= |\Phi| \mapsto \bigtimes\limits_{k\in []} \natEval{R}_\Phi(k, |\Phi|)\\
                &= |\Phi| \mapsto []\\
                &= \natEval{\vnil^\nat}_\Phi
            \end{align*}
        ...
        \begin{align*}
            \natEval{\Qrange @n @m}_\Phi &= |\Phi| \mapsto \bigtimes\limits_{i=n}^{m-1} i\\
            &=|\Phi| \mapsto \begin{cases}
                [] \text{ if } n-m = 0 \\
                n\times \bigtimes\limits_{i=n+1}^{m-1} i \text{ otherwise}
            \end{cases}\\
            &=|\Phi| \mapsto \begin{cases}
                [] \text{ if } \natEval{n-m}_\Phi = 0 \\
...
\begin{proof}
    ...
    \begin{itemize}
        ...
        \item If $M = \Qaccumap_{A,B,C}\ N'\ M_{xs}\ M_{fs}\ M_{z}$ and \(N=
            ...
            \(
                \trans{\Qaccumap_{A,B,C}\ N'\ M_{xs}\ M_{fs}\ M_{z}}_{\Phi,\Gamma,\Delta,\Pi} = |\Phi|\mapsto
            \)
            \[
                \scalebox{0.8}{\tikzfig{proof/trans-reduct-accumap-10}}
            \]
            \begin{gather*}
                \;\overset{\scriptstyle{{\bf(p,gs,sg)}}}{=}|\Phi|\mapsto\\
                \scalebox{0.8}{\tikzfig{proof/trans-reduct-accumap-20}}
            \end{gather*}
            \begin{align*}
                =\llbracket & \ifz{N'}{xs\ ;_v\ fs\ ;_v\ \vnil\otimes M_z}{}\\
                    &\qquad\Qlet{x :: xs'}{M_{xs}}{} \\
                    &\qquad\Qlet{f :: fs'}{M_{fs}}{} \\
                    &\qquad\Qlet{y\otimes z'}{f\ x\ z}{} \\
                    &\qquad\Qlet{ys \otimes z''}{\Qaccumap\ @(N'-1)\ xs'\ fs'\ z'}{} \\
                    &\qquad(y :: ys) \otimes z''\rrbracket_{\Phi,\Gamma,\Delta,\Pi}
            \end{align*}
        ...
        \item If $M = \Qsplit_A\ @n\ @m\ xs$ and \(N=
            ...
            \begin{align*}
                ...
                &\overset{\scriptstyle{{\bf(sg,gs)}}}{=}\;|\Phi|\mapsto\tikzfig{proof/trans-reduct-split-20}\\
                ...
                &= \left\llbracket\begin{array}{l}
                    \ifz{n}{\vnil\otimes xs}{}\\
                    \Qlet{y :: xs'}{xs}{} \\
                    \Qlet{ys_1\otimes ys_2}{\Qsplit @(n-1)\ @m\ xs'}{}\\
                    (y :: ys_1)\otimes ys_2
                \end{array}\right\rrbracket_{\Phi,\Gamma}
            \end{align*}
        ...
        \item If $M = \Qappend_A\ @N_1\ @N_2\ M_1\ M_2$ and \(N=
            ...
            \begin{align*}
                ...
                \Qlet{x :: xs'}{M_1}{}\\
                x :: (\Qappend\ @(N_1-1)\ @N_2\ M_1\ M_2)
                \end{array}\right\rrbracket_{\Phi,\Gamma,\Delta}
            \end{align*}
