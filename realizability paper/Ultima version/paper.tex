\documentclass[a4paper,UKenglish,numberwithinsect,cleveref,pdfa]{lipics-v2021}
...
\Cref{sec:model} presents the realisability model with unitary type semantics,
...
\begin{definition}[Basis]\label{def:NthDimensionalBasis}
  ...
  \begin{enumerate*}
    ...
    \item Each element of $B$ is an $n$-qubit state \emph{(cf.~\Cref{def:Qubit})}.
    \item Distinct elements of $B$ are pairwise orthogonal.
  \end{enumerate*}
\end{definition}

From now on, the syntax introduced in \Cref{tab:Syntax}
is restricted to sets $B$ forming $2^n$-dimensional
orthonormal bases.
...
with $\vv t\ansubst{\vv{v_i}/x}{B_1}\ansubst{\vv{w_i}/y}{B_2}$
defined for every $i$, then:
\[
...
This definition, together with \Cref{lem:BasesIso}, allows us to build a
...
\Cref{tab:TypingRules} presents a representative subset forming a reasonable
