...
The realisability methodology, provides a constructive framework that connects
...
Finally, define the typing judgement so that a type assertion equates, by definition,
to state that the term reduces to a value of that type.
...
While the untyped calculus allows for the construction of non-physical terms, 
the extracted type system acts as a logical filter: only those terms that reduce 
to unit-norm values and correspond to unitary transformations are typable.
...
$t,s,r$ for terms, writing $\vv{\cdot}$ when they are combinations.
...
$\vv v$ over the explicit basis $B$. In the second, when $B=\AbsBasis$,
substitution proceeds linearly over the basic values that form $\vv v$.  
...
distinguishes between the superposition of results, $\Lam{x}{B}{\alpha\vv{v_1}
+ \beta\vv{v_2}}$, and the superposition of functions,
$\alpha(\Lam{x}{B}{\vv{v_1}}) + \beta(\Lam{x}{B}{\vv{v_2}})$.  This distinction
...
\begin{table}[t]
  ...
  \end{align*}
  If $t\lraneq \vv r$, then
  $\left\{\begin{aligned}
    st &\lraneq s\vv r \qquad\qquad tv \lraneq \vv rv \\
    \text{If }\alpha\neq 0:\quad\vv v+\alpha t+\vv s &\lraneq \vv v+\alpha \vv r+\vv s & (*)\\
    \LetP{x}{A}{y}{B}{t}{\vv{s}} &\lraneq \LetP{x}{A}{y}{B}{\vv r}{\vv{s}} \\
    \gencase{t}{\vv{v_1}}{\vv {v_n}}{\vv{s_1}}{\vv{s_n}} &\lraneq \gencase{\vv r}{\vv{v_1}}{\vv {v_n}}{\vv{s_1}}{\vv{s_n}}
\end{aligned}\right.$
...
\begin{definition}[Type realizer]\label{def:Realizer}
  Given a term combination~$\vv t$ and type~$A$, we say that~$\vv t$
  \emph{realizes}~$A$ (not.~$\vv t \real A$) if there exists a value
...
\begin{table}[t]
  ...
  \begin{align*}
    ...
    \bigr\}\\
    \sem{\sharp Y}&:= {(\sem{Y}^\bot)}^\bot\quad\text{Where: }Y^\bot = \{\vec{v}\in\Sph\,\mid\,\scal{\vec{v}}{y} = 0, \forall y\in Y\}
  \end{align*}
...
\begin{definition}
  ...
  write
  \(
    \vv{v}\equiv \sum_{i=1}^{2^n}\alpha_i\ket{i}
  \),
