...
generalised Lambda-S to arbitrary non-entangled single-qubit bases, to first order.
The $\lambda_B$ calculus 
presented here extends these ideas to a unit-norm, higher-order setting over
multiple-qubit bases, grounded in a realisability semantics;
a comparison with previous systems is provided in Table~\ref{tab:related}.
\begin{table}
  \centering
...
proceeds as follows:
First define a calculus with a deterministic evaluation strategy.
Then, define types as sets of closed values in the language.
Finally, define the typing judgement so that asserting that a term has a given type
is, by definition, to state that the term reduces to a value of that type.
...
This work pursues two goals: first, to provide a more exact program description via the extracted type system; and second, to express quantum algorithms compositionally, rather than as mere circuit translations.
...
spiders correspond to computations relative to the computational and Hadamard
...
\item We develop a \emph{realisability-based semantics} that connects
  ...
  provable theorems in the realisability interpretation.
...
basic terms and values, respectively. 
...
\begin{table}[t]
  ...
  \begin{align*}
    ...
    \equiv
      ...
      \\
    &\gencase{\alpha \vv{t}}{\vv{v_1}}{\vv{v_k}}{\vv{s_1}}{\vv{s_k}}
    \equiv \alpha(\gencase{\vv{t}}{\vv{v_1}}{\vv{v_k}}{\vv{s_1}}{\vv{s_k}}) \\
    &\gencase{(\vv{t}+\vv{s})}{\vv{v_1}}{\vv{v_k}}{\vv{r_1}}{\vv{r_k}}
    \equiv\begin{aligned}[t]
      &\gencase{\vv{t}}{\vv{v}}{\vv{w}}{\vv{r_1}}{\vv{r_k}}\\
      &+\gencase{\vv{s}}{\vv{v}}{\vv{w}}{\vv{r_1}}{\vv{r_k}}
    \end{aligned}
  \end{align*}
...
The rules in Table~\ref{tab:Congruence} ensure that the
set of value combinations satisfies the axioms of a vector space.  The notion
...
syntax of Table~\ref{tab:Syntax}.

We will also consider the following notation for linear combinations of
pairs. This notation does not appear in the syntax,
...
\begin{definition}[Qubits]\label{def:Qubit}
  ...
  $\vv{w_1}$ and $\vv{w_2}$ are $(n-1)$-qubit states,
...
product defined by,  
\(
  \scal{\vv{v}}{\vv{w}} := \sum_{i=1}^k\sum_{j=1}^k
...
\),
where $\vv{v}=\sum_{i=1}^k\alpha_i v_i$ and
$\vv{w}=\sum_{j=1}^k\beta_j w_j$, and $\delta_{v_i,w_j}$ is the Kronecker
...
decorating basis are treated in call-by-value, while others are decomposed as
...
As expected, no non-trivial linear combination of basis
...
capture-avoiding substitution of $\vv v$ for $x$ in $\vv t$.
...
special symbol, denoted $\AbsBasis$. This symbol changes the nature of the lambda
abstraction, allowing it to take other abstractions as arguments. This is the only
instance of higher-order functions in the calculus. The symbol $\AbsBasis$, is a purely
syntactical device, intuitively it can be thought as the canonical basis. It 
distributes the application over every basic value as defined in the syntax.
So, from now on, the symbols $B, B_1, B_2$ in Table~\ref{tab:Syntax} are either $2^n$-dimensional orthonormal basis, or the special symbol $\AbsBasis$.


\begin{definition}[Basis-dependent substitution]
  Let $\vv t$ be a term combination, $\vv v$ a value combination, $x$ a
...
$\vv v\equiv\sum_{i\in I}\alpha_i\,\Pair{\vv{v_i}}{\vv{w_i}}$,
then:
\[
...
\begin{lemma}[Distributivity over linear combinations]\label{lem:distributiveSubstitution}
  For term combinations $\vv{t_i}$, a value combination $\vv{v}$, a
...
\begin{lemma}[Compatibility with congruence]\label{lem:EquivSubstitutions}
  For value combinations $\vv{v},\vv{w}$, a term combination $\vv{t}$, and
...
$\vv{t}$, closed value combinations $\vv{v_1},\dots,\vv{v_n}$, variables
...
$x\notin\dom{\sigma}$, value combination $\vv{v}$, and basis $B$,
\(
...
\begin{table}[t]
  \begin{align*}
    ...
    (\Lam{x}{X}{\vv{t}})\vv{v} \lraneq \vv{t}\ansubst{\vv v/x}{X}\\
    \text{If }\vv{t}\ansubst{\vv v/x}{X\otimes Y}\text{ is defined}:\quad&
    \LetP{x}{X}{y}{Y}{\vv v}{\vv{t}} \lraneq \vv{t}\ansubst{\vv{v}/x\otimes y}{X\otimes Y}\\
    \text{If for all } i\neq j\ \vv{v_i}\perp\vv{v_j}:\quad&
    \gencase{\sum_{i=1}^{n}\alpha_i \vv{v_i}}{\vv{v_1}}{\vv{v_n}}{\vv{t_1}}{\vv{t_n}} \lraneq \sum_{i=1}^{n}\alpha_i \vv{t_i}
  \end{align*}
  \[
    \begin{array}{c}
     ...
     % \\[5pt]
      \infer[(*)]{\vv v+\alpha t+\vv s\lraneq \vv v+\alpha \vv r+\vv s}{t\lraneq \vv r & \alpha\neq 0}
%      \infer{(\sum\limits_{i=1}^{j-1}\vv v_i) + \alpha_j t_j + (\sum\limits_{i=j+1}^{n} \alpha_i t_i) 
%      \lraneq (\sum\limits_{i=1}^{j-1}\vv v_i) + \alpha_j \vv r_j + (\sum\limits_{i=j+1}^{n} \alpha_i t_i) }
%      {t_j\lraneq \vv r_j & \text{ (*)}}
      \\[12pt]
      \infer{\LetP{x}{A}{y}{B}{t}{\vv{s}}\lraneq
      \LetP{x}{A}{y}{B}{\vv r}{\vv{s}}}{t\lraneq \vv r} 
      \\[5pt]
      \infer{\gencase{t}{\vv{v_1}}{\vv {v_n}}{\vv{s_1}}{\vv{s_n}}\lraneq
      \gencase{\vv r}{\vv{v_1}}{\vv{v_n}}{\vv{s_1}}{\vv{s_n}}}{t\lraneq \vv r}
    \end{array}
  \]
    In (*) we use the convention that the subterms in the sum are ordered as follows: first we place all the values, if any, followed by the non-value terms in a lexicographical order.
...
defined for abstractions, we introduced the symbol $\AbsBasis$ to 
linearly over the basis values of a combination. For instance,
\[
  (\Lam{x}{\AbsBasis}{\vv t})
...
\begin{theorem}[Reduction preserves equivalence]\label{thm:confluence}
  Let $\vv{t}$ and $\vv{s}$ be closed term combinations with
  $\vv{t}\equiv\vv{s}$. If $\vv{t}\lraneq\vv{t'}$ and $\vv{s}\lraneq\vv{s'}$,
  then there exist term combinations $\vv{r_1}$ and $\vv{r_2}$ such that
...
:= \{\vv v \in \vv{\Val} \mid \|\vv v\| = 1\}$, which
corresponds to the mathematical representation of quantum data as unit vectors
in a Hilbert space.

\begin{definition}[Unitary value combination]
  A value combination $\vv{v}$ is said to be \emph{unitary} if its norm equals~1,
  that is, if $\vv{v}\in\Sph$.
\end{definition}

\begin{definition}[Unitary type]
  A \emph{unitary type} (or simply a \emph{type}) is an expression~$A$ together
...
\begin{definition}[Type realizer]\label{def:Realizer}
  Given a type~$A$ and a term combination~$\vv t$, we say that~$\vv t$
  \emph{realizes}~$A$ (written~$\vv t \real A$) if there exists a value
  combination~$\vv v\in\sem A$ such that
...
\begin{table}[t]
  ...
  \begin{align*}
    ...
    \bigr\}\\
    \sem{\sharp X}&:= {(\sem{X}^\bot)}^\bot\quad\text{Where: }X^\bot = \{\vec{v}\in\Sph\,\mid\,\scal{\vec{v}}{a} = 0, \forall x\in X\}
  \end{align*}
...
of type $\sharp\basis{X}\Arr\sharp\basis{Y}$, where both bases have size~$2^n$,
represent unitary operators $\C^{2^n}\to\C^{2^n}$.
...
\begin{definition}
  Let $X$ be an orthonormal basis of size~$2^n$. For every $\vv{v}\in X$, we can
  write
  \(
    \vv{v}\equiv \sum\limits_{i=1}^{2^n}\alpha_i\ket{i}
  \),
  where $\ket{i}$ denotes the $n$-qubit tuple of $\ket{0}$ and $\ket{1}$
  corresponding to the binary representation of~$i$, and
  $\sum\limits_{i=1}^{n}|\alpha_i|^2=1$. For example, for $n=4$, $\ket{3}$ is
  $\ket{0011}$. We then define $\pi_n:X\to\C^{2^n}$ by
  \(
    \pi_n(\vv{v}) = (\alpha_1,\dotsc,\alpha_{2^n})
  \).
\end{definition}
...
\begin{proof}
  ...
  $\|\vv{w_i}\|_{\C^{2^n}}=\|\vv{w_j}\|_{\C^{2^n}}=1$ and
  $\scal{\vv{w_i}}{\vv{w_j}}_{\C^{2^n}}=0$.
  ...
  \),
  as $\|\vv{w_i}\|=\|F(\vv{v_i})\|_{\C^n}=1$. Since
  \(
    \scal{\vv{w_i}}{\vv{w_j}}
...
\begin{definition}
  ...
  the judgements
    ...
    \item for
      ...
      there exist value combinations $\vv{v},\vv{w}$ such that
...
presented in Table~\ref{tab:TypingRules}—we must
...
\begin{theorem}[Subject reduction]\label{thm:SubjectReduction}
  ...
  Table~\ref{tab:TypingRules} and $\vv{t}\to\vv{u}$, then
...
We can simulate the behaviour of this protocol in the $\lambdaB$ calculus by
...
\iffalse
Despite usually taking $\Phi^+$ for the execution of the protocol,
any of the four states can be used to perform the teleportation. So perhaps,
a more precise representation of the algorithm would be a program which takes
\emph{any} Bell state and an arbitrary qubit as argument and performs
the corresponding computations. 

.....
This program exhibits some interesting characteristics:
\begin{itemize}
  \item 
\end{itemize}


This program exhibits both classical control (when matching the Bell state),
and quantum control (when )
.....
\fi
...
\begin{proof}
  ...
  $\sum_{i=1}^{2^n} \alpha_i \vv{b_i}$ and $\sum_{i=1}^{2^n} \beta_i \vv{b_i}$
  are two decompositions of $\vv{v}$ over $B$. Then
  \[
    0=\vv{v}-\vv{v}=\sum_{i=1}^{2^n}(\alpha_i-\beta_i)\vv{b_i}.
  \]
...
\begin{proof}
  ...
  argument as in Theorem~\ref{thm:UniqueDecomposition} shows that $\vv{v}$ and $\vv{w}$ have the
...
\begin{lemma}[Weak diamond property for $\lraneq$]\label{lem:SquigDiamond}
  Let $\vv{t}, \vv{s_1}, \vv{s_2}$ term combinations such that $\vv{t}\lraneq{s_1}$ and $\vv{t}\lraneq\vv{s_2}$. 
  Then, either there exists a term combination $\vv{r}$ such that $\vv{s_1}\lraneq\vv{r}$ and 
  $\vv{s_2}\lraneq \vv{r}$. Or, $\vv{s_1}=\vv{s_2}$. Diagrammatically:
  \[
    \begin{tikzcd}
      & \vv{t}
        \arrow[ld,decorate,decoration={snake, amplitude=0.8, segment length=6pt}, ->]
...
\begin{proof}
  ...
  \begin{description}
    ...
    \item[$0\vv{t}\equiv\vv{0}$:] The term combinations cannot reduce on either side of the equivalence.
    ...
    \item[$\alpha(\vv{t_1}+\vv{t_2})\equiv\alpha\vv{t_1}+\alpha\vv{t_2}$:] If $\alpha=0$, then the term combinations cannot reduce on either side of the equivalence. Otherwise, we match the reductions on both sides of the equivalence with Lemma \ref{lem:SquigDiamond}.
...
\begin{proof}
  ...
  Then $\sum_{i=1}^n \alpha_i \vv{w_i}\in\sem{\sharp(\sharp\basis{Y})}=\sem{\sharp\basis{Y}}$ by Theorem~\ref{thm:IdempotentSharp}. Since for every $\vv u\in\sem{\sharp A}$, $(\Lam{x}{A}{\vv t}) \vv u\real\sharp B$, we can conclude that $\Lam{x}{A}{\vv t}\in\sem{\sharp A\Arr\sharp B}$.\qedhere
\end{proof}

Before proving the soundness of the typing rules (Theorem~\ref{thm:TypingRulesValidity}), we need the following results.

\begin{theorem}\label{prop:InnerProdPairs} For all value combinations $\vv{v_1}, \vv{v_2}, \vv{w_1}, \vv{w_2}$ we have:
\[
...
\begin{proof}
    ...
    When $\lambda=0$, we first observe that $\alpha\neq 0$ since it would mean that $\|\vv{u_1}\|=0$ which is absurd since $\|\vv{u_1}\|=1$. Moreover, since $\lambda=\|\vv{u_1}+\alpha\vv{u_2}\|=0$, we observe that all the coefficients of the combination $\vv{u_1}+\alpha\vv{u_2}$ are zeroes when written in canonical form which implies that:
    \[
...
Given a valid typing judgement of the term $\TYP{\Delta,x_B:\sharp A}{\vv{s}}{C}$, a substitution $\sigma\in\sem{\Delta}$ and value combinations $\vv{u_1},\vv{u_2}\in\sem{\sharp A}$, there are value combinations $\vv{w_1}, \vv{w_2}\in\sem{C}$ such that:
\[
\begin{array}{c}
...
\begin{proof}
    From the validity of the judgement of the form $\TYP{\Delta, x_A:\sharp A}{\vv{s}}{C}$, a substitution $\sigma\in\sem{\Delta}$, and value combinations $\vv{w_1},\vv{w_2}\in\sem{C}$ such that $\vv{s}\ansubst{\sigma}{}\ansubst{\vv{u_1}/x}{A}\eval\vv{w_1}$ and $\vv{s}\ansubst{\sigma}{}\ansubst{\vv{u_2}/x}{A}\eval\vv{w_2}$. In particular, we have that $\|\vv{w_1}\| = \|\vv{w_2}\|=1$. Applying Lemma~\ref{lem:VecRewrite}~four times, we know there are vectors $\vv{u_{01}},\vv{u_{02}},\vv{u_{03}},\vv{u_{04}}\in\sem{\sharp A}$ and scalars $\lambda_1,\lambda_2,\lambda_3,\lambda_4$ such that:
    \begin{align*}
...
Given a valid typing judgement of the form $\TYP{\Delta, x_{B_1}:\sharp A_1, y_{B_2}: \sharp A_2}{\vv{s}}{C}$, a substitution $\sigma\in\sem{\Delta}$ and value combinations $\vv{u_1},\vv{u_2}\in\sem{\sharp A}$, there are value combinations $\vv{w_1},\vv{w_2}\in\sem{C}$ such that:
\[
\begin{array}{c}
    \vv{s}\ansubst{\sigma}{}\ansubst{\vv{u_1}/x}{B_1}{\ansubst{\vv{v_1}/y}{B_2}}\eval\vv{w_1}\\
    \vv{s}\ansubst{\sigma}{}\ansubst{\vv{u_2}/x}{B_1}{\ansubst{\vv{v_2}/y}{B_2}}\eval\vv{w_2}\\
...
\begin{proof}
    From Lemma~\ref{lem:VecRewrite} we know that there are $\vv{u_0}\in\sem{\sharp A}, \vv{v_0}\in\sem{\sharp B}$ and $\lambda,\mu\in\C$ such that:
    \[
    ...
    \begin{enumerate}
        ...
        \item\label{A8:it3} $\scal{\vv{u_1}}{\vv{u_2}}=0$, so that from Lemma~\ref{lem:InnerProdSingleVar}~we get $\scal{\vv{w_{1k}}}{\vv{w_{2k}}}=0$ (for all $k\in\{0,1,2\}$).
        
        \item\label{A8:it4} $\scal{\vv{v_1}}{\vv{v_2}}=0$, so that from Lemma~\ref{lem:InnerProdSingleVar}~we get $\scal{\vv{w_{j1}}}{\vv{w_{j2}}}=0$ (for all $j\in\{0,1,2\}$).
    \end{enumerate}
...
Given a valid typing judgement of the form $\TYP{\Delta,x_{B_1}:\sharp A_1, y_{B_2}:\sharp A_2}{\vv{s}}{C}$, a substitution $\sigma\in\sem{\Delta}$, and value combinations $\vv{u_1},\vv{u_2}\in\sem{\sharp A}$ and $\vv{v_1},\vv{v_2}\in\sem{\sharp B}$, there are value combinations $\vv{w_1},\vv{w_2}\in\sem{C}$ such that:
\[
\begin{array}{c}
    \vv{s}\ansubst{\sigma}{}\ansubst{\vv{u_1}/x}{B_1}{\ansubst{\vv{v_1}/y}{B_2}}\eval\vv{w_1}\\
    \vv{s}\ansubst{\sigma}{}\ansubst{\vv{u_2}/x}{B_1}{\ansubst{\vv{v_2}/y}{B_2}}\eval\vv{w_2}\\
...
\begin{proof}
    ...
    \]
    And similarly that, $\scal{\vv{v_1}}{\vv{v_2}+ (-\beta) \vv{v_1}} = 0$. From Lemma~\ref{lem:VecRewrite}, we know that there are $\vv{u_0}\in\sem{\sharp A}$, $\vv{v_0}\in\sem{\sharp B}$ and $\lambda,\mu\in\C$ such that:
    \begin{align*}
    ...
    \begin{enumerate}
        ...
        \item\label{A9:it3} $\scal{\vv{u_1}}{\lambda\vv{u_0}}=\scal{\vv{u_1}}{\vv{u_2}+(- \alpha)\vv{u_1}}=0$, so that from Lemma~\ref{lem:InnerProdSingleVar} we get:
        \[
        \scal{\vv{w_{1k}}}{\lambda\vv{w_{0k}}}= 0 \qquad(\text{for all }k\in\{0,1,2\})
        \]
        
        \item\label{A9:it4} $\scal{\vv{v_1}}{\mu\vv{v_0}}=\scal{\vv{v_1}}{\vv{v_2} + (-\beta)}\vv{v_1}=0$, so that from Lemma~\ref{lem:InnerProdSingleVar} we get:
        \[
        \scal{\vv{w_{j1}}}{\mu\vv{w_{j0}}}=0
        \]
                
        \item\label{A9:it5} $\scal{\vv{u_1}}{\lambda\vv{u_0}}=\scal{\vv{v_1}}{\mu\vv{v_0}}=0$ so that from Lemma~\ref{lem:OrthogonalSubstitution} we get:
        \[
        \scal{\vv{w_{11}}}{\lambda\mu\vv{w_{00}}}=0
        \]
        (Again the equality $\scal{\vv{w_{11}}}{\lambda\mu\vv{w_{00}}}$ is trivial when $\lambda=0$ or $\mu=0$. When $\lambda ,\mu\neq 0$ we deduce from the above that $\scal{\vv{u_1}}{\vv{u_0}}=\scal{\vv{v_1}}{\vv{v_0}}=0$, from which we get $\scal{\vv{w_{11}}}{\vv{w_{00}}}=0$ by Lemma~\ref{lem:OrthogonalSubstitution})
    \end{enumerate}
...
\begin{proof}
    ...
    \begin{align*}
        ...
        &=(\sum_{i=1}^{n}\alpha_i \vv{t_i})\ansubst{\sigma}{}\ansubst{\vv v/x}{X}\qquad{\text{By Lemma~\ref{lem:distributiveSubstitution}}}
    \end{align*}
    
    Considering that $\ansubst{\sigma}{}\in\sem{\Gamma}$, then we have that $\ansubst{\sigma}{}\ansubst{\vv v/x}{X}\in\sem{\Gamma,x^X:A}$. Since we assume $\TYP{\Gamma, x^X:A}{\sum_{i=1}^{n}\alpha_i\vv{t_i}}{B}$, then $\vv{t_i}\ansubst{\sigma}{}\ansubst{\vv v/x}{X}\real B$. Finally, we can conclude that the combination: $\sum_{i=1}^{n}\alpha_i (\Lam{x}{X}{\vv{t_i}})\in\sem{A\Arr B}$.
    ...
    \begin{align*}
        ...
        &=\sum_{k=1}^{l}\sum_{k'=1}^{l}\overline{\gamma_k e^{i\rho_k}}\ \gamma_{k'} e^{i\rho_{k'}} \scal{\vv{v_k}}{\vv{v_{k'}}}\scal{\vv{u_k}}{\vv{u_{k'}}}\quad(\text{from Lemma~\ref{lem:UnitPreserTens}})\\
        &= \sum_{k=1}^{k}\sum_{k'=1}^{l}\overline{\gamma_k e^{i\rho_k}}\  \gamma_{k'} e^{i\rho_{k'}} \scal{\Pair{\vv{u_k}}{\vv{v_k}}}{\Pair{\vv{u_{k'}}}{\vv{v_{k'}}}}\quad(\text{from Theorem~\ref{prop:InnerProdPairs}})\\
