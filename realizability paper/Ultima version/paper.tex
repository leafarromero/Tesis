...
The quantum teleportation protocol~\cite{PhysRevLett.70.1895} exemplifies the transmission of quantum data via shared entanglement. In $\lambdaB$, this protocol serves to illustrate how the \emph{deferred-measurement principle}~\cite[\S 4.4]{NC00} is internalised within a purely unitary framework. While measurement can be integrated into basis-sensitive calculi (as shown in~\cite{DiazcaroMonzonAPLAS25}), our focus here is to demonstrate that the $\mathsf{case}$ constructor is sufficient to reason about classically controlled gates by branching on basis states.
...
The strength of this approach lies in the type system's ability to verify the consistency of the entire superposition: each branch is statically checked as a unitary transformation. The overall term is assigned the precise type $\sharp\basis{\B}\Arr(\sharp\basis{\Bell}\times\sharp\basis{\B})$, where $\sharp\basis{\Bell}$ captures the entanglement of the first two qubits\footnote{The type derivation relies on the subtypings 
