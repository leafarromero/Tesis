...
through a purely syntactic framework restricted to first order. The calculus $\lambdaB$ that we
...
You can see a comparison of the papers in this line, in Table~\ref{tab:related}.
...
This work pursues two goals: first to employ the extracted type
system to provide a more exact description of programs, and second, to take
advantage of the extended syntax to express quantum algorithms in a compositional manner,
rather than merely translating circuits.
...
Section~\ref{sec:conclusion} concludes with final remarks and perspectives.
...
\begin{definition}[Qubits]\label{def:Qubit}
  A \emph{1-qubit state} is a value distribution of the form
  $\alpha\ket{0} + \beta\ket{1}$, where $|\alpha|^2 + |\beta|^2 = 1$. An
  \emph{$n$-qubit state} is a value distribution of the form
...
\begin{definition}[Basis]\label{def:NthDimensionalBasis}
  A set of value distributions $B$ is a \emph{$2^n$-dimensional orthonormal
...
%in~Table~\ref{tab:UnitaryTypes} should be
...
It remains to verify that our type algebra captures the intended sets of
...
the overall judgement we can derive for the application is:
\(
...
The deferred-measurement principle (see, for example,~\cite[\S 4.4]{NC00}) states that any quantum circuit can delay
...
transformation but expressed without any explicit
...
${\sharp\basis{\B}\Arr(\sharp\basis{\Bell}\times\sharp\basis{\B})}$\footnote{The type derivation relies on the subtypings 
...
distributions decompose onto different bases.  
