...
\begin{abstract}
  ...
  Each abstraction and $\mathsf{let}$-construct is annotated with a basis,
...
\begin{definition}[Basis-dependent substitution]\label{def:basisDependentSub}
  ...
  As remarked in \ref{rem:UniqueDecompostion}, this decomposition is unique,
...
The strength of this approach lies in the type system's ability to verify the consistency of the entire superposition: each branch is statically checked as a unitary transformation. The overall term is assigned the type $\sharp\basis{\B}\Arr(\sharp\basis{\Bell}\times\sharp\basis{\B})$ (relying on the subtypings 
$\basis{\Bell}\leq\sharp(\basis{\B}\times\basis{\B})$ and
$\sharp\basis{\B}\times\sharp\basis{\B}\leq\sharp\basis{\Bell}$), which also ensures the teleported qubit will be separable.
