...
\begin{abstract}
  ...
  respect to arbitrary---possibly entangled---bases, combining quantum control 
  and realisability semantics ensuring unitarity by construction.
  Each instance of variable binding is annotated with a basis,
  and a new basis-dependent substitution governs the decomposition of value
  combinations. These extensions preserve the expressive power of earlier calculi
  while enabling finer reasoning about programs under basis changes. Realisability
  semantics connects the reduction system with the type system, yielding a direct
...
only in that case. Later, Lambda-S$_1$ \cite{DiazcaroGuillermoMiquelValironLICS19,
DiazCaroMalherbe2022} refined this approach by restricting to unit-norm vectors,
introducing higher-order abstractions, and ensuring that terms of type
qubit-to-qubit denote unitary maps. More recently, the Lambda-SX calculus 
\cite{DiazcaroMonzonAPLAS25} generalised Lambda-S to arbitrary non-entangled 
single-qubit bases, to first order. The $\lambda_B$ calculus presented here extends
these ideas to a unit-norm, higher-order setting over multiple-qubit bases,
grounded in a realisability semantics.
...
linear handling for unknown qubits. This technique works well for first-order
terms, but it fails to capture the notion of basis for higher order terms. In
case of a combination of abstractions, the system reduces linearly over it.

This work pursues two goals: first, to provide a more exact program description
via the extracted type system; and second, to express quantum algorithms 
compositionally, rather than as mere circuit translations.

With these goals in mind, we take the design choice to not include measurement and
study a pure calculus. Previous works on this line of research have proposed 
solutions to support measurement \cite{DiazcaroDowekRinaldiBIO19,
DiazcaroMonzonAPLAS25}, so we decided to narrow the focus of this paper to the 
unitary setting.
...
effect.  
Similarly,~\cite{CaretteHeunenKaarsgaardSabry2024} introduces
...
Related efforts in semantic characterisation—such as the fully
...
entanglement and partial trace. Our calculus follows the opposite philosophy: 
linearity and duplicability are syntactically restricted by the basis-sensitive 
type system, guaranteeing coherent quantum control without relying on external 
semantic constraints.

The calculi in \cite{DaveLemonnierPechouxZamdzhiev_2025, BarssePechouxPerdrix_2026} work 
on a probabilistic setting to accommodate measurement. The operational semantics can be
seen as the action of programs on quantum states and thus working on two different layers.
In contrast, our work focuses on the pure quantum fragment yielding a deterministic
calculus where state and program share the same layer.

The SYM calculus \cite{SabryValironVizzotto_2018}, is based on linear combinations of
terms in the style of \cite{ArrighiDowekLMCS17} working within the pure quantum fragment
(as $\lambdaB$). However, each calculus focuses on different objectives. SYM implements 
structural recursion and limited higher-order, without requiring norm-1 for the terms. While 
$\lambdaB$ sports less constrained higher-order functions and ensures typed terms fall in the 
unitary sphere.

A comparison with a number of calculi in the quantum-data/quantum-control paradigm is
provided in Table~\ref{tab:related}. This is by no means an exhaustive list, but
rather an overview to compare different characteristics and foci in the field.


\begin{table}
  \centering
  \begin{tabular}{|l|c|c|c|c|c|c|}\hline
    & Qunity \cite{VoichickLiRandHicks2023}
    & SYM \cite{SabryValironVizzotto_2018} 
    & \cite{DaveLemonnierPechouxZamdzhiev_2025} 
    & \cite{BarssePechouxPerdrix_2026}
    ...
    \\\hline
  \end{tabular}
  \caption{Comparison of quantum-control $\lambda$-calculi.}
  \label{tab:related}
\end{table}
\footnotetext[1]{For coherent pattern-matching.}
...
along the lines of the untyped language Lineal \cite{ArrighiDowekLMCS17}.
...
\begin{table}[t]
  \centering
  \begin{tabular}{rcll}
    $v$ & $::=$ & $x \mid \Lam{x}{X}{\vec{t}} \mid (v, v) \mid \ket{0} \mid \ket{1}$ & $(\Val)$ \\
    $t$ & $::=$ & $v \mid tt \mid (t,t) \mid \LetP{x}{X}{y}{Y}{\vec{t}}{\vec{t}} \mid \gencase{\vec{t}}{\vec{v}}{\vec{v}}{\vec{t}}{\vec{t}}$ & $(\Lambda)$ \\
  ...
  \end{tabular}
  \caption{Syntax of the calculus, where $X, Y \subseteq \ValD$.}
  \label{tab:Syntax}
\end{table}
...
This $\mathsf{case}$ operator supports quantum control which enables
computations without a definite causal structure, such as the quantum switch 
\cite{Chiribella_2013}.
...
The symbols $X$, and $Y$ appearing in the abstractions and pair
destructors denote the basis of the vector spaces in which the terms are
expressed; their precise nature is made explicit below.
Before formally defining the notion of basis, we first establish the
...
\begin{remark}
  The calculus syntax only allows for application between basic terms. The term congruence 
  is the mechanism by which we are able to apply term combinations, since:
  \[
  (\sum_{i=0}^{n} \alpha_i t_i\ ) (\sum_{j=0}^{m} \beta_j s_j\ ) \equiv
  \sum_{i=0}^{n} \sum_{j=0}^{m} \alpha_i \beta_j (t_i\ s_j)
  \]
\end{remark}
...
The presentation of the term congruence does not lend itself easily to implementation.
In order to do so, we would need to have a directed set of rules with normal forms
and extra definitions for casting. This issue was first addressed by \cite{ArrighiDowekLMCS17},
and later refined in \cite{DiazcaroDowekRinaldiBIO19} and, \cite{DiazcaroMonzonAPLAS25}.
We choose this presentation since the previous works give a satisfactory answer for
possible implementation, and it simplifies our system for analysis.
...
\begin{remark}[Unique decomposition]\label{rem:UniqueDecomposition}\label{cor:EquivalentDecomposition}
  If $B$ is a $2^n$-dimensional basis, then every $n$-qubit state has a
  unique decomposition over $B$. This means that when $\vec{v} \equiv \vec{w}$,
  then they share the same decomposition over any basis $B$.
  \qed
\end{remark}
...
Alongside this substitution, we extend the set of possible basis annotations by
including a special symbol \AbsBasis.
Since we only consider bases for
...
\ref{tab:Syntax} are either $2^n$-dimensional orthonormal bases, or the special
symbol $\AbsBasis$.
...
%    \end{cases}
%  \]
%  Notice that the uniqueness of the decomposition over $B$ is guaranteed by Remark~\ref{rem:UniqueDecomposition}.
%\end{definition}
\begin{definition}[Basis-dependent substitution]\label{def:basisDependentSub}
  Let $\vec t$ be a term combination, $\vec v$ a value combination, $x$ a
  ...
  $\vec{v}=\sum_{i\in I} \alpha_i v_i$; otherwise it is undefined.
  As remarked in \ref{rem:UniqueDecomposition}, this decomposition is unique,
  and the substitution is well-defined.
\end{definition}
...
both substitutions result in the exact same term. Note that this property
...
\begin{definition}[Unitary type]
  A \emph{unitary type} (or simply a \emph{type}) is an expression~$A$ built from a
  formal language with a set of unitary value combinations, denoted~$\sem{A}$,
  called the \emph{unitary semantics} of~$A$.
\end{definition}
...
\begin{table}[t]
  ...
  \begin{tabular}{l}
    ...
    $\sem{\sharp A}:= {(\sem{A}^\bot)}^\bot$, with $Y^\bot = \{\vec{v}\in\Sph \mid \forall y\in Y, \scal{\vec{v}}{y} = 0\}$
  \end{tabular}
...
define a concrete set of types for our language. We begin with the type grammar
...
\begin{remark}\label{rem:SharpCharacterization}\label{rem:IdempotentSharp}
  ...
  \[
    ...
    = \Span(\sem{A}) \cap \Sph
  \] 
  Moreover, as expected for a span, the~$\sharp$ operator is idempotent;
  that is, $\sem{\sharp A} = \sem{\sharp(\sharp A)}$. Proof of this
  remark can be found on the appendix.
  \qed
\end{remark}
...
\end{theorem}
\begin{proof}
  The proof is simple, we merely have to show that $\Sph$ is stable under pairings. The detailed proof can be found on the appendix.
\end{proof}

Defining types as sets of values naturally induces a semantic notion of
subtyping. The relationship is simply given by set inclusion. 
...
\begin{table}[t]
  ...
  \begin{tabular}{c@{\quad}c@{\quad}c@{\quad}c}
    \infer{\sharp (\basis{X})\cong\sharp(\basis{Y})}{\mathsf{size}(X) = \mathsf{size}(Y)} &
    \infer{\basis{X\times Y} \cong \basis{X}\times\basis{Y}}{} &
    \infer{\sharp A \leq \sharp B}{A\leq B} &
    \infer{(\sharp A)\times(\sharp B)\leq\sharp(A\times B)}{}
  \end{tabular}
  \caption{Some valid subtyping rules.}
  \label{tab:Subtyping}
\end{table}

We present some of these subtyping relations in Table~\ref{tab:Subtyping}
...
\begin{definition}
  ...
  \end{align*}
  Where $\ansubst{\--/\--}{\--}$ denotes the substitution defined
  in~\ref{def:basisDependentSub}.
\end{definition}
...
\begin{definition}
  ...
  \).\footnote{This definition relies on the idempotence of the~$\sharp$~operator (Remark~\ref{rem:IdempotentSharp}).
  }
\end{definition}
...
to orthogonal values. We therefore introduce the following notion.

\begin{definition}[Orthogonality judgement]
...
we just write $\SORTH{\Gamma}{\vec{t}}{\vec{s}}{A}$.
\end{definition}
...
In the general case, determining orthogonality between two different open terms
is potentially undecidable. This is a known problem since the first iterations
of calculi that worked with these kind of judgements \cite{AltenkirchGrattageLICS05}.
Intuitively, the semantical condition for orthogonality on the $\snam{Sum}$ and
$\snam{UnitCase}$ rules can be thought as an external proof that must be provided by
the programmer. Fortunately, the orthogonality for qubits and closed terms is easily
verified.
...
\end{align*}
This term represents the protocol by encoding the coherent superposition of outcomes and their corresponding unitary corrections ($I, Z, X, ZX$). 

The strength of this approach lies in the type system's ability to verify the 
consistency of the entire superposition: each branch is statically checked as a
unitary transformation. The overall term is assigned the type $\sharp\basis{\B}
\Arr\sharp(\basis{\Bell}\times\basis{\B})$ (relying on the subtypings 
$\basis{\Bell}\leq\sharp(\basis{\B}\times\basis{\B})$ and
$\sharp\basis{\B}\times\sharp\basis{\B}\leq\sharp\basis{\Bell}$).

Despite being the usual choice for describing the protocol, the choice of $\Phi^+$
is arbitrary. Any of the four Bell basis states is a suitable input. Expressing the
quantum teleportation in this manner, gives a structure from where we can abstract
a more general representation. Consider the following program:
\begin{align*}
\mathsf{Teleport} :=
  &\Lam{b}{\Bell}{ \Lam{x}{\B}{
  \LetP{y_1}{\B}{y_2}{\B}{b}{
  \mathsf{let}\ (c_1^{\AbsBasis}, c_2^{\AbsBasis}, 
  c_3^{\AbsBasis}, c_4^{\AbsBasis}) = (\mathsf{corr}\ b)\ \mathsf{in}\\
  &\mathsf{case}\; \Pair{x}{y_1}\; \mathsf{of} \{\;
  \Phi^{+} \mapsto \Pair{\Phi^{+}}{c_1\,y_2},\;
  \Phi^{-} \mapsto \Pair{\Phi^{-}}{c_2\,y_2},\\
	&\Psi^{+} \mapsto \Pair{\Psi^{+}}{c_3\,y_2},\;
	\Psi^{-} \mapsto \Pair{\Psi^{-}}{c_4\,y_2} \}
    }
  }
  }.
\end{align*}

We use syntactic sugar for collapsing multiple $\mathsf{let}$ and $\mathsf{corr}$ is
the classically-controlled program that outputs the appropriate correction order for
each of the Bell-basis vectors. It encodes the following function:
\[
\mathsf{corr}(x) = 
\begin{cases}
($I, Z, X, ZX$) \text{If }x=\Phi^+\qquad
($I, Z, X, ZX$) \text{If }x=\Phi^+\\
($I, Z, X, ZX$) \text{If }x=\Phi^+ \qquad
($I, Z, X, ZX$) \text{If }x=\Phi^+\\
\end{cases}
\]

Overall, the term is assigned the type $\basis{\Bell}\Arr\sharp(\basis{\B})
\Arr\sharp(\basis{\Bell}\times\basis{\B})$. The advantage here is that the function
takes \emph{any} of the four Bell basis states as argument and performs the
protocol. The resulting term is a more abstract respresentation of the protocol,
without relying on a predetermined choice of entangled pair.

In both terms, the typing system requires checking orthogonality on the branches of
the $\mathsf{case}$ constructor. In this particular case, it is a simple thing to 
do, since every branch returns a pair with four different pairwise orthogonal values
alongside the corrected qubit. 

Moreover, it is also easy to prove that \emph{no other combination} of the Bell 
basis states can be used for the protocol. This is due to the fact that the 
superposition of protocols does not preserve unitarity. A value of type 
$\sharp\basis{\Bell}$ would yield a term superposition of type $\sharp(\sharp 
(\basis{\B})\Arr\sharp (\basis{\Bell}\times\basis{\B}))$ which does not act as a 
meaningful arrow type.

In short, the calculus allows us to abstract both the encoding and decoding
steps in the Bell basis, exploiting the deferred-measurement principle. The program 
lends itself to generalisation on the Bell basis and the type system restricts the meaningful inputs.
...
A current line of work is defining categorical semantics allowing for comparison with other
established calculi, and to incorporate new features. Another, would be to extend the notion of
basis to include higher-order values; this question was tackled during the development of this
paper, but a satisfactory answer remained elusive. We hope in a future work, to generalize the
notion in order to encompass all values under a unified framework for the reduction system. 
...
\begin{restateremark}[Restatement of Remark~\ref{rem:UniqueDecomposition}]
  \itshape
...
\begin{proof}
  ...
  argument as in Remark~\ref{rem:UniqueDecomposition} shows that $\vec{v}$ and $\vec{w}$ have the same decomposition over $B$.
\end{proof}
...
\begin{restatetheorem}[Restatement of Remark~\ref{rem:SharpCharacterization}]
  \itshape
  The interpretation of a type~$\sharp A$ contains precisely the
  norm-$1$ linear combinations of values in~$\sem{A}$:
  \[
...
\begin{restatetheorem}[Restatement of Remark~\ref{rem:IdempotentSharp}]
  \itshape
...
\begin{proof}
  ...
  Then $\sum_{i=1}^n \alpha_i \vec{w}_{i}\in\sem{\sharp(\sharp\basis{Y})}=\sem{\sharp\basis{Y}}$ by Remark~\ref{rem:IdempotentSharp}. Since for every $\vec u\in\sem{\sharp A}$, $(\Lam{x}{A}{\vec t}) \vec u\real\sharp B$, we can conclude that $\Lam{x}{A}{\vec t}\in\sem{\sharp A\Arr\sharp B}$.\qedhere
\end{proof}
