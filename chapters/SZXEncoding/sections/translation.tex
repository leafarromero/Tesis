...
\begin{table}\small
    ...
    \begin{array}{c}
        ...
        \multicolumn{1}{l}{\natEval{\Qlet{x^P :: y^{\typeVec{P}{n}}}{M}{N}}_\Phi = }\\[5pt]
        \multicolumn{1}{r}{|\Phi|\mapsto \natEval{N}_{x:P, y:\typeVec{P}{n}, \Phi}(y_1,[y_2,\dots,y_n],|\Phi|)}\\[5pt]
        \multicolumn{1}{r}{\text{Where } [y_1,\dots,y_n] = \natEval{M}_\Phi(|\Phi|)}
    \end{array}
    \]
    \caption{Parameter evaluation}
    \label{tab:ParameterEval}
\end{table}
...
We will omit the brackets in our diagrams for clarity. In a similar manner to the evaluation, we define the translation $\trans{M}_{\Phi,\Gamma}$ recursively on the terms in Tables \ref{tab:TermTrans} and \ref{tab:TermTrans2}.
...
\begin{table}\small
    ...
    \[
        \begin{array}{c}
            \trans{R}_{\Phi} = n,|\Phi|\mapsto\tikzfig{judgements/linear/rotation}\\[7pt]
            \trans{M\ @N}_{\Phi,\Gamma} = |\Phi|\mapsto \tikzfig{judgements/linear/applyP}\\[7pt]
...
\[
    \begin{array}{c}
        \trans{M \otimes N}_{\Phi, \Gamma, \Delta} = |\Phi|\mapsto \scalebox{0.9}{\tikzfig{judgements/linear/product}}
        \\[14pt]
        \trans{M;_v N}_{\Phi, \Gamma, \Delta} = |\Phi|\mapsto \scalebox{0.9}{\tikzfig{judgements/linear/then}}\\[14pt]
        \trans{let\ x^A \otimes y^B = M\ in\ N}_{\Phi, \Gamma, \Delta} = |\Phi|\mapsto \scalebox{0.9}{\tikzfig{judgements/linear/let}}
        \\[15pt]
        \trans{let\ x^A : y^{\typeVec{A}{n}} = M\ in\ N}_{\Phi, \Gamma, \Delta} = |\Phi|\mapsto \scalebox{0.9}{\tikzfig{judgements/linear/letVec}}\\[10pt]
        \trans{M\!::\!N}_{\Phi, \Gamma, \Delta} = |\Phi|\mapsto \scalebox{0.9}{\tikzfig{judgements/linear/vector}}
        \\[17pt]
        \trans{\Qfor{k}{V}{M}}_{\Phi, \Gamma^n} = |\Phi| \mapsto \scalebox{0.9}{\tikzfig{judgements/primitives/for}}
        \\[17pt]
        \trans{\ifz{L}{M}{N}}_{\Phi, \Gamma} = |\Phi| \mapsto \scalebox{0.9}{\tikzfig{judgements/linear/ifz}}
    \end{array}    
\]
\caption{State translation (cont.)}
\label{tab:TermTrans2}
\end{table}
